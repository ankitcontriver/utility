...
4. **Low Confidence Detection**:  
   - If the system detects that the confidence level for intent matching is lower than 0.3, return:
     ...
     ```
   - This ensures the user is aware that the system could not match their query accurately, and they can rephrase or refine their request.
...
6. **Analyzing User Input**:  
   ...
   - If confirmation is needed, return:
     ...
     ```
   - Upon receiving confirmation or denial, update the `input_confirmed` field to `"success"` or `"failure"` based on the user's response.

7. **Context Awareness**:  
   - Always check the **previous and current query** to analyze the user’s intent more accurately. For example:
