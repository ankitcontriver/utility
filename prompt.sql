...
4. Parent-Child Node Differentiation: You need to emphasize that if a query is specific to a child node (e.g., "weekly bundles"), the system should always prioritize that over parent nodes (e.g., "data offers"). You can add a rule that explicitly prevents returning parent nodes in such cases.
5. If user asks to directly take you to a certain node id number, return the -1 node id.
